...
BUSINESS_LOG_AUDIENCE = "application_detail"
...
def add_business_log(
    ...
    *,
    ...
    """Write a user-facing application log while retaining the full technical log stream."""

    metadata: dict[str, Any] = {
        "audience": BUSINESS_LOG_AUDIENCE,
        "event": "business_progress",
    }
    if progress is not None:
        metadata["progress"] = max(0, min(progress, 100))
    return add_job_log(
        job,
        level,
        message,
    ...
    *,
    ...
    if include_logs:
        ...
        if log_view == "business":
            logs = logs.filter(metadata__audience=BUSINESS_LOG_AUDIENCE)
