...
from apps.workflow.Jobs.http import JobIntegrationCallRecorder, format_log_value
from apps.workflow.Jobs.models import Job
from apps.workflow.Jobs.services import add_job_log
...
class JobIntegrationObserver:
    ...
    def __init__(self, job: Job) -> None:
        self._job = job
        self._handles: dict[int, tuple[JobIntegrationCallRecorder, object]] = {}
        self._next_handle = 1
    ...
    def request_started(
        ...
        *,
        ...
        recorder = JobIntegrationCallRecorder(self._job, step=step)
        inner = recorder.record_started(
            method=method,
            path=url,
            headers=headers,
            request_body=body,
        ...
        self._next_handle += 1
        self._handles[handle] = (recorder, inner)
        return handle
    ...
    def request_finished(
        ...
        *,
        ...
        recorder, inner = self._handles.pop(int(str(handle)))
