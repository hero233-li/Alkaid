...
from apps.workflow.Jobs.models import TERMINAL_JOB_STATUSES, Job
from apps.workflow.Jobs.services import BUSINESS_LOG_AUDIENCE, serialize_log
...
def _snapshot(job_id: int, after_id: int, log_view: str = "all") -> dict[str, Any] | None:
    close_old_connections()
    try:
        job = Job.objects.get(id=job_id)
    except Job.DoesNotExist:
        return None
    logs_query = job.logs.filter(id__gt=after_id)
    if log_view == "business":
        logs_query = logs_query.filter(metadata__audience=BUSINESS_LOG_AUDIENCE)
    logs = list(logs_query.order_by("id")[:501])
...
async def _load_snapshot(
    job_id: int, after_id: int, log_view: str = "all"
) -> dict[str, Any] | None:
    return await sync_to_async(_snapshot, thread_sensitive=True)(job_id, after_id, log_view)
...
class JobLogSSEApplication:
    ...
    async def __call__(self, scope: dict[str, Any], receive: ASGIReceive, send: ASGISend) -> None:
        ...
        try:
            ...
            return
        log_view = query.get("view", ["all"])[0].strip().lower()
        if log_view not in {"all", "business"}:
            await self._json_error(send, 400, "view 参数无效")
            return
        snapshot = await _load_snapshot(job_id, after_id, log_view)
        if snapshot is None:
