...
class CjdkJyrcAdapter:
    """Translate neutral product commands/results at the CJDK boundary."""
    ...
    def __init__(
        ...
        *,
        ...
        self._client = CjdkJyrcClient(
            settings=settings,
            observer=observer,
            trace_id=trace_id,
            environment=environment,
        )
        self._gateway = JavaApplicationLinkGateway(observer=observer, trace_id=trace_id)
        self._total_document_bytes = 0

    @property
    def application_link_url_mode(self) -> str:
        return self._settings.application_link_url_mode

    def __enter__(self) -> CjdkJyrcAdapter:
    ...
    def generate_application_link(
        ...
        links = self._gateway.generate_link(request)
    ...
    def query_agreement_templates(
        ...
        if len(templates) > limit:
            raise AgreementProtocolError(
                f"协议模板返回 {len(templates)} 个，超过上限 {limit}"
            )
        return tuple(
            AgreementTemplateResult(
                doc_id=item.doc_id,
    ...
    def query_agreement_preview(
        ...
        if len(documents) > limit:
            raise AgreementProtocolError(
                f"协议预览返回 {len(documents)} 个文档，超过上限 {limit}"
            )
    ...
    def read_agreement_document(self, doc_id: str) -> AgreementDocumentResult:
        ...
        self._total_document_bytes += content_bytes
        total_limit = self._settings.response_limits.max_total_document_bytes
        if self._total_document_bytes > total_limit:
            raise AgreementProtocolError(
                f"所有协议文档累计超过 {total_limit} bytes"
            )
...
# Temporary import compatibility while callers migrate to the combined adapter.
CjdkJyrcAgreementAdapter = CjdkJyrcAdapter
