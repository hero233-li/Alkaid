...
def compile_application_link_plan(
    *,
    ...
    if normalized_environment not in product.environments:
        raise ApplicationLinkPlanError(f"产品 {product.code} 不支持环境：{normalized_environment}")
    ...
    if not matches:
        raise ApplicationLinkPlanError(
    ...
    try:
        ...
        raise ApplicationLinkPlanError(str(exc)) from exc
    ...
    return CompiledApplicationLinkPlan(
...
def validate_catalog_application_link_plans(catalog: Any) -> None:
    ...
    if len(route_ids) != len(set(route_ids)):
        ...
        raise ApplicationLinkPlanError(f"申请链接 routeId 重复：{duplicate}")
