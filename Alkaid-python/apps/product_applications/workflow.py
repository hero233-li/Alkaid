...
def required_text(source: Mapping[str, Any], key: str, label: str | None = None) -> str:
...
def build_product_application_result(
    ...
    *,
    ...
    return {
        ...
        "message": "申请链接、Session、协议及身份验证完成",
        "applicationLink": {
            "generated": True,
...
def execute_product_application(
    *,
    ...
    with runtime:
        application_session = runtime.open_application(
            snapshot=snapshot,
            application_link_kind=application_link_kind,
            progress=progress,
        )
        agreement_reading = runtime.read_agreements(
            payload=submission.payload,
            progress=progress,
        )
        application_submission = runtime.submit_application(
            payload=submission.payload,
            progress=progress,
        )
        runtime.verify_identity(
            payload=submission.payload,
            environment=required_text(submission.payload, "environment", "环境"),
            submission=application_submission,
            progress=progress,
        )
    return build_product_application_result(
        ...
        snapshot,
        outcome={
            "applicationLinkCategory": application_session["applicationLinkCategory"],
            "selectedApplicationLinkKind": application_session["selectedApplicationLinkKind"],
            "agreementTemplates": agreement_reading["agreementTemplates"],
            "agreementPreview": agreement_reading["agreementPreview"],
            "agreementDocuments": agreement_reading["agreementDocuments"],
