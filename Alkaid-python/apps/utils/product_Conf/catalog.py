...
CONFIG_ROOT = Path(__file__).parents[2] / "config" / "products"
PRODUCT_ROOT = CONFIG_ROOT
...
class ProductWorkflowStep(BaseModel):
    """One versioned business module selected by a product."""
...
class ProductCatalogSource(BaseModel):
    ...
    workflow: tuple[ProductWorkflowStep, ...] = Field(min_length=1)
    ...
    def validate_local_references(self) -> ProductCatalogSource:
        ...
        workflow_step_ids = [step.step_id for step in self.workflow]
        if len(workflow_step_ids) != len(set(workflow_step_ids)):
            raise ValueError("产品流程步骤 id 不能重复")
...
class ProductExecutionSnapshot(BaseModel):
    ...
    workflow: tuple[ProductWorkflowStep, ...]
...
def _load_product_catalog(
    ...
    try:
        ...
        for path in sorted(product_root.glob("product_*.json")):
            raw = _read_json(path)
