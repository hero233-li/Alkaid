...
const submission = (id: number) => ({ id, status: 'pending' as const, stage: 'pending', progress: 0 });
...
describe('useVerificationApproval', () => {
  beforeEach(() => {
    ...
    vi.mocked(api.pollVerificationJob)
      ...
      .mockResolvedValueOnce(detail(2, { ...baseTask, items: [{ ...baseTask.items[0], status: 'completed' }] }))
      .mockResolvedValueOnce(detail(3, { ...baseTask, taskStatus: '已刷新', items: [{ ...baseTask.items[0], status: 'completed' }] }));
  });
  ...
  it('automatically refreshes with the complete returned context after an item update', async () => {
    ...
    expect(api.refreshVerificationTask).toHaveBeenCalledWith(expect.objectContaining({
      id: 'VERIFY-1',
