...
export default function DynamicResultModal({
  result,
  onClose,
}: {
  ...
  const title = result ? (
    <Space>
      ...
      <Tag color="blue">
        {result.functionCode} · {result.functionName}
      </Tag>
    </Space>
  ) : (
    '查询结果'
              ...
              .map((column) => {
                ...
                return (
                  <Descriptions.Item key={column.key} label={column.title}>
                    {column.type === 'tag' ? (
                      <Tag color={column.tagColors?.[text] || 'processing'}>{text}</Tag>
                    ) : (
                      text
                    )}
                  </Descriptions.Item>
