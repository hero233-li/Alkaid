...
export default function HighFrequencyWorkflowModal({
  activity,
}: {
    ...
    >
      {activity && (
        <>
          ...
          <Progress percent={activity.progress ?? 0} status="active" />
          {activity.currentStep && (
            <Typography.Text type="secondary">当前步骤：{activity.currentStep}</Typography.Text>
          )}
        </>
      )}
    </Modal>
