...
import type { CardSearchValues } from '../types';
export default function CardSearchForm({
  form,
  busy,
  onSubmit,
}: {
  form: FormInstance<CardSearchValues>;
  busy: boolean;
  onSubmit: (v: CardSearchValues) => void;
}) {
  return (
    <Card title="卡状态查询">
      <Form
        ...
        layout="vertical"
        disabled={busy}
        initialValues={{ environment: cardStatusConfig.environments[0] }}
        onFinish={onSubmit}
      >
        <Row gutter={16}>
          <Col span={12}>
            <Form.Item name="environment" label="环境" rules={[{ required: true }]}>
              <Select
                options={cardStatusConfig.environments.map((value) => ({ value, label: value }))}
              />
            </Form.Item>
          </Col>
          <Col span={12}>
