    ...
    <div className="task-center-log-list">
      {logs.map((log, index) => (
        <div className="task-center-log-line" key={log.id ?? `${log.createdAt}-${index}`}>
          <div className="task-center-log-meta">
            <Typography.Text type="secondary">{formatTime(log.createdAt)}</Typography.Text>
            <Tag
              color={log.level === 'ERROR' ? 'error' : log.level === 'WARN' ? 'warning' : 'blue'}
            >
              {log.level}
            </Tag>
          </div>
          <div className="task-center-log-message">{log.message}</div>
        </div>
      ))}
    </div>
  );
        ...
        {detail ? (
          <Space direction="vertical" size={16} className="task-center-detail-stack">
            ...
            <Card size="small" title="请求信息">
              <Descriptions size="small" column={2} bordered>
                <Descriptions.Item label="Workflow ID">{detail.workflowId}</Descriptions.Item>
                <Descriptions.Item label="执行次数">{detail.attemptCount}</Descriptions.Item>
                <Descriptions.Item label="幂等键">{detail.idempotencyKey}</Descriptions.Item>
                <Descriptions.Item label="超时时间">{detail.timeoutSeconds} 秒</Descriptions.Item>
                <Descriptions.Item label="截止时间" span={2}>
                  {formatTime(detail.deadlineAt)}
                </Descriptions.Item>
              </Descriptions>
              <Card size="small" title="请求参数" className="task-center-request-payload">
                <JsonContent value={detail.payload ?? {}} />
              </Card>
            </Card>
