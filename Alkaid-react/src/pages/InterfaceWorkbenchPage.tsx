  ...
  return (
    <div className="workbench-apifox">
      <WorkbenchHistoryPanel
        ...
        visiblePackages={packages.visibleItems}
        selectedPackageRequestId={packages.selectedRequestId}
        packagesLoading={packages.loading}
        sazImporting={packages.importing}
        onImportSaz={(file) => void packages.importSaz(file)}
        onOpenPackageRequest={(packageId, requestId) =>
          void packages.openRequest(packageId, requestId)
        }
        onDeletePackage={(packageId) => void packages.remove(packageId)}
        onRefresh={() => {
          void history.load();
          void packages.load();
        }}
