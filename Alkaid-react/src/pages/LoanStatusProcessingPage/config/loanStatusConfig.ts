...
export const loanStatusConfig = {
  ...
  actions: [
    ...
    ['freeze', '冻结'],
    ['unfreeze', '解冻'],
  ].map(([value, label]) => ({ value: value as LoanAction, label })),
};
export const loanActionLabel = (action: LoanAction) =>
  loanStatusConfig.actions.find((item) => item.value === action)?.label ?? action;
