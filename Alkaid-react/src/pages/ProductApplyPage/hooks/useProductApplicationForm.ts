...
export function useProductApplicationForm(
  ...
  useEffect(() => {
    ...
    const normalizedValues = normalizeProductApplicationValues(config, { ...initialValues, ...draft });
    form.resetFields();
  ...
  const handleValuesChange = useCallback((
    ...
    for (const fieldName of config.cascadeResetMap[changedName] || []) {
      ...
      const nextField = buildProductApplicationFields(config, nextValues)
  ...
  const createSubmission = useCallback((values: ProductApplicationFormValues) => {
    if (!config) {
