...
export default function ReleaseManagementPage() {
  ...
  const saveItem = async () => {
    ...
    try {
      ...
      message.error(
        error instanceof Error ? error.message : editingItem ? '修改版本失败' : '新增版本失败',
      );
    } finally {
      setSaving(false);
    }
          ...
          },
          {
            ...
            render: (value: string) => (
              <Typography.Paragraph
                className="release-table-content"
      ...
      >
        <Form form={form} layout="vertical" requiredMark={false}>
          <Form.Item
            name="version"
            label="版本号"
            rules={[{ required: true, message: '请输入版本号' }, { max: 100 }]}
          >
