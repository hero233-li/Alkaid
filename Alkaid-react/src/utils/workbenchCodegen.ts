...
function activeFormFields(payload: WorkbenchRequestPayload) {
  return (payload.formFields || []).filter((field) => field.enabled !== false && field.name?.trim());
}

function headerEntries(payload: WorkbenchRequestPayload, skipContentType = false) {
  return Object.entries(payload.headers || {}).filter(([name]) => !skipContentType || name.toLowerCase() !== 'content-type');
}
...
function generateJavascriptCode(payload: WorkbenchRequestPayload) {
  ...
  if (payload.bodyMode === 'form-data') {
    ...
    activeFormFields(payload).forEach((field) => {
      if (field.type === 'file') {
        lines.push(`// formData.append(${stringLiteral(field.name)}, fileInput.files[0], ${stringLiteral(field.fileName || 'file')});`);
      } else {
        lines.push(`formData.append(${stringLiteral(field.name)}, ${stringLiteral(field.value || '')});`);
      }
    });
    ...
    if (fields.length) {
      ...
      fields.forEach((field) => {
        if (field.type !== 'file') {
          lines.push(`body.append(${stringLiteral(field.name)}, ${stringLiteral(field.value || '')});`);
        }
      });
...
function generateJavaCode(payload: WorkbenchRequestPayload) {
  ...
  const publisher = body ? 'HttpRequest.BodyPublishers.ofString(body, StandardCharsets.UTF_8)' : 'HttpRequest.BodyPublishers.noBody()';
  const lines = [
    'import java.net.URI;',
  ...
  if (payload.bodyMode === 'form-data') {
    lines.push('', '// form-data 文件字段需要按本地文件流组装 multipart body，这里先保留字段清单。');
    activeFormFields(payload).forEach((field) => {
      const value = field.type === 'file' ? `@${field.fileName || '<file>'}` : field.value || '';
  ...
  lines.push(`    .method(${stringLiteral(payload.method)}, ${payload.bodyMode === 'form-data' ? 'HttpRequest.BodyPublishers.noBody()' : publisher})`);
  lines.push('    .build();', '');
  lines.push('HttpResponse<String> response = client.send(request, HttpResponse.BodyHandlers.ofString(StandardCharsets.UTF_8));');
...
function generatePythonCode(payload: WorkbenchRequestPayload) {
  ...
  const lines = ['import requests', '', `url = ${stringLiteral(payload.url)}`, `headers = ${pythonDict(headerEntries(payload, skipContentType))}`];
  const fields = activeFormFields(payload);

  if (payload.bodyMode === 'form-data') {
    const textFields = fields.filter((field) => field.type !== 'file').map((field) => [field.name, field.value || ''] as [string, string]);
    ...
    if (fileFields.length) {
      ...
      fileFields.forEach((field) => {
        ...
        lines.push(`    ${stringLiteral(field.name)}: (${stringLiteral(fileName)}, open(${stringLiteral(fileName)}, "rb")),`);
      });
      lines.push('}');
      lines.push(`response = requests.request(${stringLiteral(payload.method)}, url, headers=headers, data=data, files=files)`);
    } else {
      lines.push(`response = requests.request(${stringLiteral(payload.method)}, url, headers=headers, data=data)`);
    }
  } else if (payload.bodyMode === 'form-urlencoded') {
    const dataFields = fields.filter((field) => field.type !== 'file').map((field) => [field.name, field.value || ''] as [string, string]);
    if (dataFields.length) {
      lines.push(`data = ${pythonDict(dataFields)}`);
      lines.push(`response = requests.request(${stringLiteral(payload.method)}, url, headers=headers, data=data)`);
    } else {
      lines.push(`payload = ${stringLiteral(payload.body || '')}`);
      lines.push(`response = requests.request(${stringLiteral(payload.method)}, url, headers=headers, data=payload)`);
    }
  } else if (payload.bodyMode !== 'none' && payload.body) {
    lines.push(`payload = ${stringLiteral(payload.body)}`);
    lines.push(`response = requests.request(${stringLiteral(payload.method)}, url, headers=headers, data=payload)`);
  } else {
    lines.push(`response = requests.request(${stringLiteral(payload.method)}, url, headers=headers)`);
  }
...
function generateHttpCode(payload: WorkbenchRequestPayload) {
  ...
  const body = payload.bodyMode === 'form-data'
    ? activeFormFields(payload)
        .map((field) => `${field.name}=${field.type === 'file' ? `@${field.fileName || '<file>'}` : field.value || ''}`)
        .join('\n')
    : requestBody(payload);
  if (body) {
    lines.push('', body);
...
export function generateRequestCode(payload: WorkbenchRequestPayload, language: RequestCodeLanguage) {
